...
def saveCookies(message):
    ...
    loginInfo = loginEntrance(session)
    qrInfo, qr_base64 = generateQrcode(session, loginInfo)
    message.put({'qr_base64': qr_base64,'okl_token':qrInfo['okl_token']})
    headers = getHeaders(session, loginInfo, qrInfo)
    pt_key, pt_pin = formatCookie(headers)
    ...
    cookiesFileInfo, thepath = getCookieFilesInfo(pt_pin, pt_key, filePath)

    saveFiles(cookiesFileInfo, thepath, env['masterPtPin'])
