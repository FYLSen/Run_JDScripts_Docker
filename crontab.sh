...
5 8,14,20 11-17 5 * bash -c "exec 1<>/proc/1/fd/1; exec 2<>/proc/1/fd/2; set -o allexport; source /overall; source /env; source /cookies; node /scripts/z_mother_jump.js > /logs/z_mother_jump.log 2>&1 | sed 's/^/ 【母婴跳一跳】/'; "
