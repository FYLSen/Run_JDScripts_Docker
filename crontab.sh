...
0,1,30,31 * * * * bash -c "exec 1<>/proc/1/fd/1; exec 2<>/proc/1/fd/2; set -o allexport; source /cookies; node /scripts/jd_live_redrain.js > /logs/jd_live_redrain.js.log 2>&1 | sed 's/^/ 【整点红包雨】/'; "
58 6 * * * bash -c "exec 1<>/proc/1/fd/1; exec 2<>/proc/1/fd/2; set -o allexport; source /overall; source /cookies; node /scripts/jd_exchangejxbeans.js > /logs/jd_exchangejxbeans.js.log 2>&1 | sed 's/^/ 【京豆-》喜豆】/'; "
...
43 12,14,16 * * * bash -c "exec 1<>/proc/1/fd/1; exec 2<>/proc/1/fd/2; set -o allexport; source /Run_JDScripts_Docker/ShareCodes/Default.sh; source /overall; source /env; source /cookies; node /scripts/jd_dreamFactory_help.js > /logs/jd_dreamFactory_help.js.log 2>&1 | sed 's/^/ 【京喜工厂互助】/'; "
...
11 1,8,13,16,21 * * * bash -c "exec 1<>/proc/1/fd/1; exec 2<>/proc/1/fd/2; set -o allexport; source /Run_JDScripts_Docker/ShareCodes/Default.sh; source /overall; source /cookies; node /scripts/jd_health.js > /logs/jd_health.js.log 2>&1 | sed 's/^/ 【东东健康社区】/'; "
...
17 1,2 * * * bash -c "exec 1<>/proc/1/fd/1; exec 2<>/proc/1/fd/2; set -o allexport; source /Run_JDScripts_Docker/ShareCodes/Default.sh; source /overall; source /env; source /cookies; node /scripts/jd_lottery_drew.js > /logs/jd_lottery_drew.js.log 2>&1 | sed 's/^/ 【京东抽奖红包】/'; "
...
41 1,8,13,16,21 * * * bash -c "exec 1<>/proc/1/fd/1; exec 2<>/proc/1/fd/2; set -o allexport; source /overall; source /env; source /cookies; ts-node /scripts/jd_health.ts > /logs/jd_health.ts.log 2>&1 | sed 's/^/ 【健康社区】/'; "
...
3 0,15 * * * bash -c "exec 1<>/proc/1/fd/1; exec 2<>/proc/1/fd/2; set -o allexport; source /overall; source /env; source /cookies; ts-node /scripts/jd_jinbang.ts > /logs/jd_jinbang.ts.log 2>&1 | sed 's/^/ 【京东金榜】/'; "
# JS + TS 脚本
